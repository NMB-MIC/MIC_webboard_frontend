...
class CheckInPercentage extends Component {
  constructor(props) {
    super(props);
  ...
  getData = async () => {
    ...
    if (this.state.reportType === "bus") {
      resultBackend = await httpClient.post(
        server.BUS_CHECK_IN_PERCENTAGE_URL,
        {
      ...
      resultBackend = await httpClient.post(
        server.BREAK_CHECK_IN_PERCENTAGE_URL,
        {
          startDate: this.state.startDate,
          toDate: this.state.toDate,
        }
      ...
      resultBackend = await httpClient.post(server.CHECK_IN_PERCENTAGE_URL, {
        startDate: this.state.startDate,
        toDate: this.state.toDate,
      });
    }
  ...
  render() {
    ...
    for (
      ...
      if (
        this.state.reportType === "bus" ||
        this.state.reportType === "break"
      ) {
    ...
    // chart option
    let chartOption;
    if (this.state.reportType === "bus" || this.state.reportType === "break") {
      chartOption = {
        responsive: true,
          ...
          <div className="container-fluid">
            <div className="row mb-2">
              <div className="col-sm-6">
                <h1 className="m-0 text-dark">Report check in percentage</h1>
              </div>
            </div>
          </div>
        </div>
              ...
              >
                <div className="overlay-wrapper">
                  ...
                  <section className="content-header">
                    <h1>
                      Chart
                      <small> Total check in percentage</small>
                    </h1>
                    <hr></hr>
                    <div className="row">
...
export default CheckInPercentage;
